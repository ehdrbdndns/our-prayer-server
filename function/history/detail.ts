...
async function handleGet({
  ...
  try {
    const [rows]: any = await promisePool.query(`
    SELECT
        prayer_history_id, duration, note
...
async function handlePost({
  ...
  session: Session, req: { prayer_history_id_list: string[] }
}) {
  const { user_id } = session;
  const { prayer_history_id_list } = req;

  if (!prayer_history_id_list || prayer_history_id_list.length === 0) {
    return {
      statusCode: 400,
      headers: {
        "Access-Control-Allow-Origin": "*",
  ...
  try {
    ...
    SELECT 
        ...
        , UNIX_TIMESTAMP(created_date) AS created_date
    FROM prayer_history
    WHERE prayer_history_id IN (?)
      AND user_id = ?
    `, [prayer_history_id_list, user_id]);
